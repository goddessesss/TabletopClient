                ...
                >
                  ...
                  <div className="mb-3 d-flex flex-wrap gap-2">
                    {(bg.categories || []).map((c, i) => (
                      <Badge key={`${bg.name}-category-${c.name}-${i}`} bg="warning" className="me-2">
                  ...
                  <ul className="list-unstyled small mb-3" style={{ fontSize: "0.9rem", color: "#555" }}>
                    <li><FaCalendarAlt className="me-2" /><strong>Year:</strong> {bg.yearPublished || "Unknown"}</li>
                    <li><FaUsers className="me-2" /><strong>Players:</strong> {bg.minPlayers} – {bg.maxPlayers}</li>
                  </ul>
                  ...
                  {game.explanation && (
                    <div style={{ fontSize: "0.9rem", color: "#444" }}>
                      ...
                      {Array.isArray(game.explanation.sharedCategories) && game.explanation.sharedCategories.length > 0 && (
                        <p><FaTags className="me-2" /><strong>Shared Categories:</strong> {game.explanation.sharedCategories.join(", ")}</p>
                      )}
                      {Array.isArray(game.explanation.sharedMechanics) && game.explanation.sharedMechanics.length > 0 && (
                        <p><FaDice className="me-2" /><strong>Shared Mechanics:</strong> {game.explanation.sharedMechanics.join(", ")}</p>
                      )}
                      {Array.isArray(game.explanation.sharedThemes) && game.explanation.sharedThemes.length > 0 && (
                        <p><FaLayerGroup className="me-2" /><strong>Shared Themes:</strong> {game.explanation.sharedThemes.join(", ")}</p>
                      )}
                      {Array.isArray(game.explanation.sharedSubcategories) && game.explanation.sharedSubcategories.length > 0 && (
                        <p><FaPuzzlePiece className="me-2" /><strong>Shared Subcategories:</strong> {game.explanation.sharedSubcategories.join(", ")}</p>
                      )}

                      <p><FaUsers className="me-2" /><strong>Shared Family:</strong> {game.explanation.sharedFamily || "None"}</p>
                    </div>
                  )}
                </Card.Body>
              </Card>
